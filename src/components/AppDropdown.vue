...
<template lang="pug">
.z-10.hidden.bg-white.divide-y.divide-gray-100.rounded-lg.shadow.flex(:id="dropdownId" class="dark:bg-gray-700")
  ...
  ul.flex.flex-col.py-2.text-lg.text-gray-700(v-else class="dark:text-gray-200" aria-labelledby="dropdownDefaultButton")
    li.flex(v-for="menu in dropdownMenus" :key="menu.title")
      a.flex-auto.block.px-4.py-2.whitespace-nowrap.cursor-pointer.flex.items-center(
        ...
        FontAwesomeIcon(v-if="menu.fa_icon" :icon="menu.fa_icon" )
        .rounded-full.w-16.h-16.bg-cover.bg-center(v-if="menu.image" :style="{ backgroundImage: 'url(' + menu.image + ')' }")
        .pl-1 {{ menu.title }}
</template>
